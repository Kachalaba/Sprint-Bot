...
def calc_total_pr(
    previous_best: float | int | str | timedelta | None,
    current_total: float | int | str | timedelta,
) -> TotalPRResult:
    """Return total PR status comparing new total with the previous best."""

    return _detect_total_pr(previous_best, current_total)


def calc_segment_prs(
    previous_bests: Sequence[float | int | str | timedelta | None],
    new_segments: Sequence[float | int | str | timedelta],
) -> list[bool]:
    """Return list of flags showing which segments improved."""

    return list(_detect_segment_prs(previous_bests, new_segments))
...
def calc_sob(
    ...
    return _calc_sob(previous_bests, new_segments)
