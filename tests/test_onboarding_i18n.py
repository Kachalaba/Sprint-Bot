...
def _collect_step_texts() -> list[str]:
    role_label = t("onb.roles.trainer")
    return [
        t("onb.choose_role"),
...
def test_onboarding_texts_ru() -> None:
    ...
    try:
        assert _collect_step_texts() == [
            ...
            "Мы сохраняем имя, роль, группу и тренера, чтобы команда видела ваш прогресс. Вы действуете как <b>Тренер</b> и согласны на обработку этих данных?",
            "Введите ваше имя и фамилию (2-64 символа, без спецсимволов).",
            "Если тренер прислал код или ссылку — вставьте её. Можно указать ID тренера или пропустить шаг.",
...
def test_onboarding_texts_uk() -> None:
    ...
    try:
        assert _collect_step_texts() == [
            ...
            "Ми зберігаємо ім'я, роль, групу та тренера, щоб команда бачила ваш прогрес. Ви дієте як <b>Тренер</b> та погоджуєтесь із обробкою цих даних?",
            "Введіть ваше ім'я та прізвище (2-64 символи, без спецсимволів).",
            "Якщо тренер надіслав код або посилання — вставте його. Також можна вказати ID тренера або пропустити крок.",
