...
Sprint-Bot пишет структурированные JSON-логи через `utils.logger`. По умолчанию записи попадают в stdout (в Docker они доступны через `docker compose logs bot`). Пример строки:

```json
{"ts": "2024-03-19T08:15:27.123456+00:00", "level": "INFO", "msg": "command_complete", "user_id": 123456789, "cmd": "/start", "latency_ms": 42.5}
```

Стандартные поля: `ts` — ISO-время в UTC, `level`, `msg`, а также `user_id`, `cmd` и `latency_ms`, если они известны. Чтобы сохранять логи на диск, примонтируйте каталог `./logs` (уже описан в `docker-compose.yml`) и перенаправьте вывод контейнера: `docker compose logs -f bot > logs/bot.log`.

## Sentry

Интеграция включается при наличии переменной `SENTRY_DSN` в `.env`. Достаточно скопировать `.env.example`, добавить строку `SENTRY_DSN=https://<ключ>@o<org>.ingest.sentry.io/<project>` и перезапустить бот (`docker compose up -d`). После инициализации в Sentry отправляются необработанные исключения из обработчиков (см. `handlers/error_handler.py`), при этом в тегах добавляются версия бота и окружение (`ENV`), а в scope — `user.id`, если бот знает пользователя.

## Тихие часы

Тихие часы задаются переменной `QUIET_HOURS` в формате `HH:MM-HH:MM` (например, `QUIET_HOURS=22:00-07:00`). Во время тихого окна уведомления не отправляются сразу: `notifications.send_notification` складывает их в `asyncio.Queue`, а бэкграунд-задача `notifications.drain_queue()` проверяет очередь каждые `QUIET_QUEUE_INTERVAL` секунд (по умолчанию 60) и выпускает сообщения, когда окно завершается. Часовой пояс можно переопределить переменной `QUIET_HOURS_TZ` (по умолчанию `Europe/Kyiv`).

## CI

GitHub Actions выполняет пайплайн `.github/workflows/ci.yml`: линтер `ruff`, статический анализатор `mypy` и тесты `pytest` с покрытием. Перед коммитом прогоняйте локально `pip install -r requirements.txt` и `pytest -q`. Если в CI всплывает флейковый тест, перезапустите конкретный job в интерфейсе GitHub только после успешного локального `pytest -q`; при повторном падении создайте issue или добавьте стабилизацию теста.
...
2. Соберите и запустите сервис:
   ...
   Контейнер монтирует `./db` и `./logs`, поэтому база и журналы переживают перезапуск.

**Обновление**

1. Обновите код (например, `git pull`).
...
- `SENTRY_DSN`, `ENV` — настройки мониторинга ошибок.
- `QUIET_HOURS`, `QUIET_QUEUE_INTERVAL`, `QUIET_HOURS_TZ` — параметры тихих часов.
- `S3_BACKUP_BUCKET`, `S3_BACKUP_PREFIX`, `S3_STORAGE_CLASS`, `S3_ENDPOINT_URL` — резервные копии.
- `BACKUP_INTERVAL_HOURS`, `CHAT_DB_PATH` — дополнительные настройки бэкапов.
...
Sprint-Bot умеет анализировать повороты в стилях **брасс** и **баттерфляй**. Для
каждого заплыва бот агрегирует тайминги підходу, контакта зі стінкою,
відштовхування та підводної фази, а потім обчислює індекс ефективності за
допомогою `TurnService`. На основі норм для конкретного стилю користувач
отримує персональні підказки, як покращити техніку поворотів.

Щоб побачити динаміку, скористайтеся командою `/turn_analysis`. За замовчуванням
аналізуються останні результати поточного спортсмена (базовий стиль можна
змінити змінною оточення `TURN_ANALYSIS_DEFAULT_STROKE`), але тренери можуть
вказати ID підопічного:

```text
/turn_analysis breaststroke
/turn_analysis butterfly 123456789
```

У відповідь бот надсилає три графіки (середній час поворотів, порівняння
останнього тренування та теплова карта стабільності), текстовий підсумок із
кращими/гіршими поворотами та рекомендації для подальших занять.
