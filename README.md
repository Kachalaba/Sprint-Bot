...
1. Скопируйте файл `.env.example` в `.env` и заполните значения переменных окружения.
...
3. При изменении настроек обновляйте файл `.env` и перезапускайте сервис:

   ```bash
   docker compose up -d
   ```

Контейнер автоматически смонтирует локальные каталоги `./db` и `./logs` для хранения базы данных и журналов.
